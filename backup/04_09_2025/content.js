      ...
      $(container)
        ...
        .each((i, group) => {
          ...
          if (slideParent.length) {
            additionalCSS += `--_max-width: min(${getCSSInt(slideParent, 'width') - 48}px, 500px);`
          }
          ...
          if ($(container).offset().top - (blockContainer.offset().top + blockContainer.height()) <= 50) {
          ...
          if ($(document).width() - $(group).width() <= 10) {
            ...
            (getCSSInt(group, 'padding-left') > 0 || getCSSInt(group, 'left') > 0) &&
...
function generateHotspotInfoContainer() {
  $(".rlc-hotspot").each((i, el) => {
    ...
    additionalCSS += `--_max-width: ${
      $(el).closest(":where(.rlc-copygroup, .rlc-copygroup-in, .rlc-textgroup, .rlc-textgroup-in, .rlc-product-tile, .rlc-50-50__block, .rlc-block)")
        .width() - 60
    }px;`

    if (slideParent.length) {
      if (getCSSInt(slideParent, "padding-left") > 0) {
        additionalCSS += `left: ${getCSSInt(slideParent, "padding-left") + 20}px;`
      }
      ...
      // if (slideParent.width() > $(window).width() / 3) {
      //   const paddingRight = getCSSInt(slideParent, "padding-right")
      //   additionalCSS += `
      //     --_link-width: calc(${slideParent.css("width")} - 40px ${paddingRight ? "- " + paddingRight : ""});`
      // } else {
      //   additionalCSS += `--_link-width: calc(100% - 60px);`
      // }
    }
...
function generateBGLinkInfoContainer() {
  $(".rlc-bg_link").each((i, el) => {
    ...
    let additionalCSS = 'style="'
    if (slideParent.length) {
      $(el).css("--_max-width", `${slideParent.width() - getCSSInt(slideParent, 'padding-right') - 40}px`)
    }
    additionalCSS = '"'
  ...
  $(".rlc-links, .rlc-copygroup, .rlc-textgroup").each((i, linksContainer) => {
    ...
    $(linksContainer)
      ...
      .each((j, el) => {
        ...
        const containerClasses = '.rlc-copygroup, .rlc-copygroup-in, .rlc-textgroup, .rlc-textgroup-in, .rlc-product-tile, .rlc-50-50__block, .rlc-block'
        ...
        if($(linksContainer).width() > $(window).width() / 1.2){
          ...
          adContainerCSS += `--_link-width: ${$(linksContainer).width() - 40 - getCSSInt(linksContainer, "padding-right")}px;`
        } else if (
            $(linksContainer).closest('article').attr('class')?.includes('hero') ||
        ...
        } else if ($(el).offset().left - $(linksContainer).offset().left <= 50) {
          adContainerCSS += "left: 0; --_translateX: 0;"
        } else {
          adContainerCSS += "left: 50%; --_translateX: -50%;"
        }

        if($(el).closest(':where(.rlc-copygroup, .rlc-copygroup-in, .rlc-textgroup, .rlc-textgroup-in)[class*="pos-top"]').length){
...
function generateQuickShopInfoContainer() {
  $(".rlc-target.is-quick-shoppable.rlc-qs_ready").each((i, el) => {
    ...
    if(parentEl){
      const leftOffset = Math.abs($(el).offset().left - $(parentEl).offset().left)
      const rightOffset =  Math.abs(($(el).offset().left + $(el).width()) - ($(parentEl).offset().left + $(parentEl).width()))
      const topOffset = Math.abs($(el).offset().top - $(parentEl).offset().top)
      const bottomOffset =  Math.abs(($(el).offset().top + $(el).height()) - ($(parentEl).offset().top + $(parentEl).height()))
...
function getAB(el) {
  ...
  if(cgid?.includes('search')){
    cgid = `<span class='rlc-dev-err'>${urlPart.split('search?')[1].split('&')[0]}</span>`
  }

  // PID handling
  if(cgid?.includes('.html')){
    pid = cgid.split('-').at(-1).split('.html')[0]
    productColor = decodeURIComponent(urlPart.split('?')[1].split('?')[0].split('&')[0].split('colorname=')[1])
    productColor = productColor === 'undefined' && null
    cgid = null
  } 
  const filter = tagging?.includes('prefn') 
    ? tagging.split('?')[0] 
    : null
  const filterName = filter ? filter.split('&')[0].split('=')[1] : null
  const filterValue = filter ? filter.split('&')[1].split('=')[1] : null
  const abTagging = urlPart?.includes('ab=') 
    ? urlPart.split('ab=')[1] 
    : null
  ...
  return  `
    ...
    ${pid ? '<br>' : ''}
    ${productColor ? "- Color: " + productColor + "<br>" : ""}
    ${filter ? "- Filter: " + filterName + " = " + filterValue + "<br>" : ""}
    - Tag: ${abTagging}
  `
}
...
function getFontDetails(el) {
  return `
   family: ${window.getComputedStyle(el).getPropertyValue("font-family")} <br>
...
function hideFont() {
  const groupClasses = '.rlc-copygroup, .rlc-copygroup-in, .rlc-textgroup, .rlc-textgroup-in, .rlc-intro, .rlc-catslider-hd, .rlc-inner-catslider-hd'

  $(`:where(${groupClasses}) > .rlc-ab-container`).hide()
  $(`*:has(>.rlc-title:not(:where(${groupClasses}) .rlc-title)) > .rlc-ab-container`).hide()
  $(`*:has(>.rlc-dek:not(:where(${groupClasses}) .rlc-dek)) > .rlc-ab-container`).hide()
...
function showFont() {
  ...
  $(`*:has(>.rlc-title:not(:where(${groupClasses}) .rlc-title)) > .rlc-ab-container`).show()
  $(`*:has(>.rlc-dek:not(:where(${groupClasses}) .rlc-dek)) > .rlc-ab-container`).show()
