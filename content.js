...
// const window.isPLP = !!$(".plp-content-slot").length;
if (typeof window.isPLP === "undefined") {
  window.isPLP = !!$(".plp-content-slot").length;
}

if (typeof window.caid === "undefined") {
  window.caid = window.isPLP
    ? ":where(.plp-content-slot, .ingrid)"
    : "#main .open-html-content:has(.rlc-creative_v3)";
}

// let caid = window.isPLP
//   ? ":where(.plp-content-slot, .ingrid)"
//   : "#main .open-html-content:has(.rlc-creative_v3)";

if (typeof window.devHelperState === "undefined") {
...
if (typeof window.helperList === "undefined") {
...
function bindEvent() {
  ...
  $(document).on("click.rlcDevHelper", ".rlc-dev-missing-image", function () {
    ...
    if (isSwiper) {
      // const swiperEl = $(`${target}`).closest('.swiper-wrapper')
      // const slide = $(`${target}`).closest('.rlc-slide')

      // swiperEl.removeAttr('style')
      // swiperEl.css('transform', `translate3d(-${slide.offset().left - slide.width()}px, 0px, 0px)`)
...
function checkMissingImage(caid) {
  ...
  $(`${caid} img`).each(function (i) {
    ...
    if (img[0].naturalWidth === 0) {
      ...
      if (parentEl.hasClass("rlc-slide")) {
        // parentEl = parentEl.closest(".rlc-slidewrapper");
  ...
  setTimeout(showMissingImages, 250);
}
